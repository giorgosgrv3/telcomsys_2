...
fc = 2.0 #carrier frequency
T = 1.0 #symbol period
over = 10 #oversampling factor
A_pulse = 5 #half duration of pulse in symbol periods
...
#define symbols s0,s1,s2,s3,s4
symbols = array([-1-1j, 1-1j, 1+1j, -1-1j, 1+1j])
N_sym = len(symbols) #5 

#generate g(t)
phi, t_phi = gen_srrc(T, over, A_pulse, alpha)

#baseband s(t)
s_impulse = upsample(symbols, over) #impulse train sum_i{s(t)*δ(t-iT)}
...
# 1. Baseband (Complex)
subplot(2, 4, 1)
plot(t_s, real(s_baseband), 'b', linewidth=0.7)
plot(t_s, imag(s_baseband), 'r', linewidth=0.7)
title('baseband s(t)')
xlabel('Time (s)')

# 2. Passband (Real)
subplot(2, 4, 2)
plot(t_s, s_bp, 'b', linewidth=0.7)
title('bandpass sBP(t)')
xlabel('Time (s)')

# 3. Downconverted (Complex)
subplot(2, 4, 3)
plot(t_s, real(r_unfiltered), 'b', linewidth=0.7)
plot(t_s, imag(r_unfiltered), 'r', linewidth=0.7)
title('downconverted r(t)')
xlabel('Time (s)')

# 4. Filtered (Complex)
subplot(2, 4, 4)
plot(t_y, real(y_filtered), 'b', linewidth=0.7)
plot(t_y, imag(y_filtered), 'r', linewidth=0.7)
title('filtered r(t)')
xlabel('Time (s)')
...
# 5. |S(f)|
subplot(2, 4, 5)
plot(f_s, abs(S_f), 'b', linewidth=0.7)
title('|S(f)|')
xlabel('Freq (Hz)')
xlim(-fc*3, fc*3) 
...
# 6. |sBP(f)|
subplot(2, 4, 6)
plot(f_x, abs(X_f), 'b', linewidth=0.7)
title('|sBP(f)|')
xlabel('Freq (Hz)')
xlim(-fc*3, fc*3)
...
# 7. |R(f)|
subplot(2, 4, 7)
plot(f_r, abs(R_f), 'b', linewidth=0.7)
...
# 8. |Y(f)|
subplot(2, 4, 8)
plot(f_y, abs(Y_f), 'b', linewidth=0.7)
title('filtered |R(f)|')
xlabel('Freq (Hz)')
xlim(-fc*3, fc*3)
